...
## Tagline
Policy-first autonomous onchain financial agent on Base — natural language goals to structured, gated execution with Dynamic server wallets and multi-provider routing
...
### Problem
Most “agent” finance apps either (a) let models call tools with weak guardrails, or (b) hard-code a single DEX with no shared policy or audit trail. Reviewers cannot see a clear path from intent → decision → wallet action, and agent payments rarely use a documented wallet pattern.

### Solution
Flitzr is a **policy-first** agent runtime on **Base (8453)**:

1. **Structured pipeline** — Intent → deterministic policy → provider routing → explicit approval → signing → settlement / audit. No silent broadcasts.

2. **Multi-provider routing** — Bankr (agent/wallet ops), Definitive Flash (DCA/limit), Uniswap Trading API (Base spot), under one policy engine and idempotent execution ledger.

3. **Dynamic Server Wallets** — Documented agent wallet pattern (API token auth, developer-owned wallets). The agent can sign and support payment flows (x402-compatible readiness) after policy gates. User wallets remain separate via SIWE sessions.
...
- Preview-first safety model (policy + approval before state change)
...
- **Job:** Agent evaluates policy-gated financial intents, then uses Dynamic to sign / prepare payment actions on Base
- **Evidence:** `lib/dynamic.ts` · `GET /api/dynamic/status` · README Dynamic section · recorded demo of intent → policy → Dynamic readiness/action
