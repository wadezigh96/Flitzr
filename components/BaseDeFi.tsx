...
export default function BaseDeFi() {
  ...
  const [provider] = useState<'uniswap'>('uniswap')
  ...
  return <section className="card" style={{ marginTop: 16 }}>
    <div className="sectionHead"><div><div className="sectionKicker">05 · BASE DEFI HUB</div><h2>Swap · Tokenize</h2></div><span className="liveSmall">BASE · 8453</span></div>
    <p className="muted">Base-native control surface powered by the Uniswap API for market swaps and tokenized assets. Every executable transaction is explicitly signed by the connected Privy wallet.</p>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginTop: 14 }}>{(['swap','tokenize'] as const).map(item => <button key={item} onClick={() => setTab(item)} className={tab === item ? 'approveButton' : 'secondaryButton'}>{item === 'swap' ? '⇄ Swap' : '◎ Tokenize'}</button>)}</div>
