...
export async function POST(request: Request) {
  try {
    ...
    if (idempotencyKey) {
      ...
      if (!claim.sameExecution) return NextResponse.json({ error: 'This Idempotency-Key is already associated with another execution.' }, { status: 409 })
      if (!claim.claimed && claim.existingExecution && ['submitted', 'confirmed'].includes(claim.existingExecution.state)) return NextResponse.json({ execution: claim.existingExecution, alreadySubmitted: true, idempotentReplay: true })
    }
    if (['submitted', 'confirmed'].includes(execution.state)) return NextResponse.json({ execution, alreadySubmitted: true, idempotentReplay: Boolean(idempotencyKey) })
    ...
    if (execution.provider !== 'definitive' || !quoteId || !typedData) return NextResponse.json({ error: 'A valid Definitive quote and signing payload are required.' }, { status: 409 })
    let recovered: string
    try {
      recovered = await recoverTypedDataAddress({ domain: typedData.domain as any, types: typedData.types as any, primaryType: typedData.primaryType as any, message: typedData.message as any, signature: userSignature as `0x${string}` })
    } catch {
    ...
    try {
      ...
      const next = { ...submitted, ...(providerOrderId ? { providerOrderId } : {}), ...(txHash ? { txHash } : {}) }
      await executionStore.update(next)
      await executionStore.addAudit(auditEvent(next.id, 'submitted', { provider: next.provider ?? 'definitive', quoteId, providerOrderId: providerOrderId ?? null, txHash: txHash ?? null, idempotencyKey: idempotencyKey || null }))
