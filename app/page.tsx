...
export default function Home() {
  ...
  return (
    <main className="shell">
      <AgentMarketplace />
      <nav className="nav"><div className="brandWrap"><div className="brandMark">F</div><div><div className="brand">FLITZR</div><div className="brandSub">DEFENCIAL AUTONOMOUS FINANCE</div></div></div><div className="navRight"><div className="live"><span /> BASE MAINNET · LIVE</div><button className="walletButton" onClick={authenticated ? logoutWallet : connectWallet} disabled={!ready}>{authenticated ? `${shortWallet} · Disconnect` : shortWallet}</button></div></nav>
      <section className="hero"><div><div className="eyebrow"><span className="eyebrowDot" /> DEFENCIAL AUTONOMOUS FINANCE</div><h1>Tell Flitzr.<br /><span>Defencial checks it.</span><br />You stay in control.</h1><p>Natural language in. A controlled execution plan out. Flitzr separates intent, Defencial, approval, routing, wallet signing, and settlement.</p><div className="heroSignals"><span>AI AGENT</span><b>→</b><span>DEFENCIAL</span><b>→</b><span>WALLET</span><b>→</b><span>BASE</span></div></div><div className="heroPanel"><div className="panelLabel">FLITZR CONTROL PLANE</div><div className="systemRow"><span className="pulse" /><strong>{authenticated ? 'Wallet authenticated' : 'Agent operational'}</strong></div><div className="systemMeta">Preview-first · Defencial-gated · Base Mainnet</div></div></section>
